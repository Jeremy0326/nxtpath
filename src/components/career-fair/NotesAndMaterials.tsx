...
interface NotesAndMaterialsProps {
  notes: Note[];
  onAddNote: () => void;
...
export function NotesAndMaterials({
  notes,
  onAddNote,
  onEditNote,
  onDeleteNote
}: NotesAndMaterialsProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-medium text-gray-900">Notes & Collected Materials</h2>
        <button 
          onClick={onAddNote}
        ...
        {notes.map((note) => (
          ...
          >
            ...
            <div className={`p-4 ${expandedId === note.id ? 'block' : 'hidden'}`}>
              ...
              <div className="flex items-center justify-between">
                <p className="text-xs text-gray-500">Added on {note.date}</p>

...
import { getJobs, applyForJob, getSavedJobs, saveJob, unsaveJob } from '@/lib/api';
import { cvAnalysisService } from '@/services/cvAnalysisService';
import type { JobSearchParams } from '@/services/jobService';
import type { SavedJob } from '@/types/models';
...
function JobSearchPageContent() {
  ...
  const [jobToApply, setJobToApply] = useState<ExtendedJob | null>(null);
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSavedDrawerOpen, setIsSavedDrawerOpen] = useState(false);
  const [activeResume, setActiveResume] = useState<{ filename: string; url: string } | null>(null);

  const [aiJobs, setAiJobs] = useState<ExtendedJob[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (jobsNeedingVector.length > 0) {
        ...
        aiJobsWithApplied = aiJobsWithApplied.map((job: ExtendedJob) => {
          const vectorJob = vectorScores.find(vj => vj.id === job.id);
          if (vectorJob && typeof (vectorJob as any).vector_score === 'number') {
            return { ...job, vector_score: (vectorJob as any).vector_score };
          }
          return job;
        });
      }
  ...
  const handleSaveJob = async (job: ExtendedJob) => {
  ...
  const handleJobSelect = (job: ExtendedJob) => {
    setSelectedJob(job);
  };
  ...
  const handleApplyClick = async (job: ExtendedJob) => {
    setJobToApply(job);
    try {
